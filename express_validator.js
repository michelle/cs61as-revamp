...
var expressValidator = function(req, res, next) {
    ...
    req.updateParam = function(name, prop, value) {
    ...
    if (this.body && undefined !== this.body[name][prop]) {
      return this.body[name][prop] = value;
    }
    return false;
  };
  ...
  req.onValidationError = function(errback) {
    ...
    req.filter = function(param, prop) {
    var self = this;
    var filter = new Filter();
    filter.modify = function(str) {
      this.str = str;
      self.updateParam(param, prop, str); // Replace the param with the filtered version
    };
    return filter.sanitize(this.param(param)[prop]);
  };
